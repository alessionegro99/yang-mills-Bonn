...
long (*sisp_and_t_to_si_compute)(long sisp, int t, Geometry const *const geo) =
    &lexeosp_and_t_to_lexeo; // single index spatial and time -> single index
                             // tot
void (*si_to_sisp_and_t_compute)(long *sisp, int *t, long si,
                                 Geometry const *const geo) =
    &lexeo_to_lexeosp_and_t; // single index tot -> single index spatial and
                             // time
...
void init_geometry(Geometry *geo, int insize[STDIM]) {
  ...
  err = posix_memalign((void **)&(geo->d_timeslice), (size_t)INT_ALIGN,
  ...
  err = posix_memalign((void **)&(geo->d_spacecomp), (size_t)INT_ALIGN,
                       (size_t)geo->d_volume * sizeof(long));
  if (err != 0) {
    fprintf(stderr, "Problems in allocating the geometry! (%s, %d)\n", __FILE__,
            __LINE__);
  ...
  err = posix_memalign((void **)&(geo->d_tsp), (size_t)INT_ALIGN,
                       (size_t)geo->d_size[0] * sizeof(long *));
  if (err != 0) {
    fprintf(stderr, "Problems in allocating the geometry! (%s, %d)\n", __FILE__,
            __LINE__);
    exit(EXIT_FAILURE);
  ...
  for (r = 0; r < geo->d_size[0]; r++) {
    err = posix_memalign((void **)&(geo->d_tsp[r]), (size_t)INT_ALIGN,
                         (size_t)geo->d_space_vol * sizeof(long));
    if (err != 0) {
      fprintf(stderr, "Problems in allocating the geometry! (%s, %d)\n",
              __FILE__, __LINE__);
      exit(EXIT_FAILURE);
...
long cartsp_to_lexsp(int const *const ccsp, Geometry const *const geo) {
  // the index for the spatial cartesian coord. goes from 0 to STDIM-2 hence
  // ccsp[STDIM-1] cc   = t x1 x2 ... x_{STDIM-1} ccsp =   x1 x2     x_{STDIM-1}
  int i;
  long ris, aux;
...
void lexeo_to_lexeosp_and_t(long *lexeosp, int *t, long lexeo,
                            Geometry const *const geo) {
