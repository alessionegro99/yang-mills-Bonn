...
void init_gauge_conf(Gauge_Conf *GC, Geometry const *const geo,
  ...
  err = posix_memalign((void **)&(GC->lattice), (size_t)DOUBLE_ALIGN,
                       (size_t)geo->d_volume * sizeof(GAUGE_GROUP *));
  if (err != 0) {
  ...
  for (r = 0; r < (geo->d_volume); r++) {
    err = posix_memalign((void **)&(GC->lattice[r]), (size_t)DOUBLE_ALIGN,
                         (size_t)STDIM * sizeof(GAUGE_GROUP));
    if (err != 0) {
      fprintf(stderr, "Problems in allocating the lattice! (%s, %d)\n",
              __FILE__, __LINE__);
      exit(EXIT_FAILURE);
...
void read_gauge_conf(Gauge_Conf *GC, Geometry const *const geo,
  ...
  } else // read the txt header of the configuration
  {
    ...
    if (dimension != STDIM) {
      fprintf(stderr,
              "The space time dimension of the configuration (%d) does not "
              "coincide with the one of the global parameter (%d)\n",
              dimension, STDIM);
      exit(EXIT_FAILURE);
    }
    ...
    for (i = 0; i < STDIM; i++) {
      ...
      if (tmp_i != geo->d_size[i]) {
        fprintf(stderr, "The size of the configuration lattice does not "
                        "coincide with the one of the global parameter\n");
        exit(EXIT_FAILURE);
      }
    }
  ...
  if (fp == NULL) {
    ...
    if (strncmp(md5sum_old, md5sum_new, 2 * MD5_DIGEST_LENGTH + 1) != 0) {
      fprintf(stderr,
              "The computed md5sum %s does not match the stored %s for the "
              "file %s (%s, %d)\n",
...
void write_conf_on_file(Gauge_Conf const *const GC, Geometry const *const geo,
                        GParam const *const param) {
...
// compute the md5sum of the configuration and save it in res, that is a
// char[2*MD5_DIGEST_LENGTH]
void compute_md5sum_conf(char *res, Gauge_Conf const *const GC,
                         Geometry const *const geo) {
  MD5_CTX mdContext;
...
void alloc_polycorr_stuff(Gauge_Conf *GC, Geometry const *const geo,
                          GParam const *const param) {
  int i, j, err;

  err = posix_memalign((void **)&(GC->ml_polycorr), (size_t)DOUBLE_ALIGN,
                       (size_t)NLEVELS * sizeof(TensProd **));
  if (err != 0) {
    ...
    for (i = 0; i < NLEVELS; i++) {
      err = posix_memalign((void **)&(GC->ml_polycorr[i]), (size_t)DOUBLE_ALIGN,
                           (size_t)(geo->d_size[0] / param->d_ml_step[i]) *
                               sizeof(TensProd *));
      if (err != 0) {
        ...
        for (j = 0; j < (geo->d_size[0] / param->d_ml_step[i]); j++) {
          err = posix_memalign((void **)&(GC->ml_polycorr[i][j]),
                               (size_t)DOUBLE_ALIGN,
                               (size_t)geo->d_space_vol * sizeof(TensProd));
          if (err != 0) {
            fprintf(stderr,
                    "Problems in allocating ml_polycorr[%d][%d] (%s, %d)\n", i,
                    j, __FILE__, __LINE__);
            exit(EXIT_FAILURE);
          }
        }
  ...
  err =
      posix_memalign((void **)&(GC->loc_poly), (size_t)DOUBLE_ALIGN,
                     (size_t)(geo->d_size[0] / param->d_ml_step[NLEVELS - 1]) *
                         sizeof(GAUGE_GROUP *));
  if (err != 0) {
    ...
    for (i = 0; i < geo->d_size[0] / param->d_ml_step[NLEVELS - 1]; i++) {
      err = posix_memalign((void **)&(GC->loc_poly[i]), (size_t)DOUBLE_ALIGN,
                           (size_t)geo->d_space_vol * sizeof(GAUGE_GROUP));
      if (err != 0) {
        fprintf(stderr, "Problems in allocating loc_poly (%s, %d)\n", __FILE__,
                __LINE__);
        exit(EXIT_FAILURE);
...
void free_polycorr_stuff(Gauge_Conf *GC, Geometry const *const geo,
                         GParam const *const param) {
  int i, j;
...
void write_polycorr_on_file(Gauge_Conf const *const GC,
  ...
  fp = fopen(param->d_ml_file,
             "ab"); // open the configuration file in binary mode
  if (fp == NULL) {
    fprintf(stderr, "Error in opening the file %s (%s, %d)\n", param->d_ml_file,
            __FILE__, __LINE__);
    exit(EXIT_FAILURE);
    ...
    for (j = 0; j < geo->d_size[0] / param->d_ml_step[0]; j++) {
      for (i = 0; i < (geo->d_space_vol); i++) {
        err = print_on_binary_file_bigen_TensProd(fp,
                                                  &(GC->ml_polycorr[0][j][i]));
        if (err != 0) {
          fprintf(stderr, "Error in writing the file %s (%s, %d)\n",
...
void read_polycorr_from_file(Gauge_Conf const *const GC,
  ...
  if (fp == NULL) {
    ...
    if (loc_space_vol != geo->d_space_vol) {
      fprintf(stderr,
              "Error: space_vol in the multilevel file %s is different from "
              "the one in the input (%s, %d)\n",
              param->d_ml_file, __FILE__, __LINE__);
      exit(EXIT_FAILURE);
    }
  }
  ...
  if (fp == NULL) {
    ...
    for (j = 0; j < geo->d_size[0] / param->d_ml_step[0]; j++) {
      for (i = 0; i < (geo->d_space_vol); i++) {
        err = read_from_binary_file_bigen_TensProd(fp,
                                                   &(GC->ml_polycorr[0][j][i]));
        if (err != 0) {
          fprintf(stderr, "Error in reading the file %s (%s, %d)\n",
                  param->d_ml_file, __FILE__, __LINE__);
  ...
  if (strncmp(md5sum_old, md5sum_new, 2 * MD5_DIGEST_LENGTH + 1) != 0) {
    fprintf(stderr,
            "The computed md5sum %s of the multilevel file does not match the "
            "stored %s\n",
...
// compute the md5sum of the ml_polycorr[0] arrays and save it in res, that is a
// char[2*MD5_DIGEST_LENGTH]
void compute_md5sum_polycorr(char *res, Gauge_Conf const *const GC,
                             Geometry const *const geo,
                             GParam const *const param) {
  MD5_CTX mdContext;
  unsigned char c[MD5_DIGEST_LENGTH];
        ...
        for (n2 = 0; n2 < NCOLOR; n2++) {
          for (n3 = 0; n3 < NCOLOR; n3++) {
            for (n4 = 0; n4 < NCOLOR; n4++) {
              double complex dc =
                  (GC->ml_polycorr[0][j][i]).comp[n1][n2][n3][n4];
              if (endian() == 0) {
                SwapBytesDoubleComplex(&dc);
              }
              MD5_Update(&mdContext, &dc, sizeof(double complex));
...
void alloc_tube_disc_stuff(Gauge_Conf *GC, Geometry const *const geo,
                           GParam const *const param) {
  int i, err;

  alloc_polycorr_stuff(GC, geo, param);

  err = posix_memalign((void **)&(GC->ml_polyplaq), (size_t)DOUBLE_ALIGN,
                       (size_t)NLEVELS * sizeof(TensProd *));
  if (err != 0) {
    ...
    for (i = 0; i < NLEVELS; i++) {
      err = posix_memalign((void **)&(GC->ml_polyplaq[i]), (size_t)DOUBLE_ALIGN,
                           (size_t)geo->d_space_vol * sizeof(TensProd));
      if (err != 0) {
...
// compute the md5sum of the ml_polycorr[0] and ml_polyplaq[0] arrays and save
// it in res, that is a char[2*MD5_DIGEST_LENGTH]
void compute_md5sum_tube_disc_stuff(char *res, Gauge_Conf const *const GC,
                                    Geometry const *const geo,
                                    GParam const *const param) {
  MD5_CTX mdContext;
  unsigned char c[MD5_DIGEST_LENGTH];
...
void alloc_tube_conn_stuff(Gauge_Conf *GC, Geometry const *const geo,
                           GParam const *const param) {
  int i, err;

  alloc_tube_disc_stuff(GC, geo, param);

  err = posix_memalign((void **)&(GC->ml_polyplaqconn), (size_t)DOUBLE_ALIGN,
                       (size_t)NLEVELS * sizeof(TensProd *));
  if (err != 0) {
    ...
    for (i = 0; i < NLEVELS; i++) {
      err = posix_memalign((void **)&(GC->ml_polyplaqconn[i]),
                           (size_t)DOUBLE_ALIGN,
                           (size_t)geo->d_space_vol * sizeof(TensProd));
      if (err != 0) {
  ...
  err = posix_memalign((void **)&(GC->loc_plaqconn), (size_t)DOUBLE_ALIGN,
                       (size_t)geo->d_space_vol * sizeof(GAUGE_GROUP));
  if (err != 0) {
...
void free_tube_conn_stuff(Gauge_Conf *GC, Geometry const *const geo,
                          GParam const *const param) {
  int i;
...
// compute the md5sum of the ml_polycorr[0], ml_polyplaq[0] and
// ml_polyplaqconn[0] arrays and save it in res, that is a
// char[2*MD5_DIGEST_LENGTH]
void compute_md5sum_tube_conn_stuff(char *res, Gauge_Conf const *const GC,
                                    Geometry const *const geo,
                                    GParam const *const param) {
  MD5_CTX mdContext;
  unsigned char c[MD5_DIGEST_LENGTH];
    ...
    for (n1 = 0; n1 < NCOLOR; n1++) {
      for (n2 = 0; n2 < NCOLOR; n2++) {
        for (n3 = 0; n3 < NCOLOR; n3++) {
          for (n4 = 0; n4 < NCOLOR; n4++) {
            double complex dc =
                (GC->ml_polyplaqconn[0][i]).comp[n1][n2][n3][n4];
            if (endian() == 0) {
              SwapBytesDoubleComplex(&dc);
            }
            MD5_Update(&mdContext, &dc, sizeof(double complex));
...
void alloc_clover_array(Gauge_Conf *GC, Geometry const *const geo) {
  int i, err;
  long r;

  err = posix_memalign((void **)&(GC->clover_array), (size_t)DOUBLE_ALIGN,
                       (size_t)geo->d_volume * sizeof(GAUGE_GROUP **));
  if (err != 0) {
    ...
    for (r = 0; r < geo->d_volume; r++) {
      err = posix_memalign((void **)&(GC->clover_array[r]),
                           (size_t)DOUBLE_ALIGN, STDIM * sizeof(GAUGE_GROUP *));
      if (err != 0) {
      ...
      for (i = 0; i < STDIM; i++) {
        err = posix_memalign((void **)&(GC->clover_array[r][i]),
                             (size_t)DOUBLE_ALIGN, STDIM * sizeof(GAUGE_GROUP));
        if (err != 0) {
...
void end_clover_array(Gauge_Conf *GC, Geometry const *const geo) {
  int i;
  long r;
...
void init_higgs_conf(Gauge_Conf *GC, Geometry const *const geo,
                     GParam const *const param) {
  long r;
  int err;

  // allocate the higgs field
  err = posix_memalign((void **)&(GC->higgs), (size_t)DOUBLE_ALIGN,
                       (size_t)geo->d_volume * sizeof(GAUGE_VECS));
  if (err != 0) {
  ...
  err = posix_memalign((void **)&(GC->Qh), (size_t)DOUBLE_ALIGN,
                       (size_t)geo->d_volume * sizeof(FMatrix));
  if (err != 0) {
  ...
  err = posix_memalign((void **)&(GC->Dh), (size_t)DOUBLE_ALIGN,
                       (size_t)geo->d_volume * sizeof(double complex));
  if (err != 0) {
...
void read_higgs_conf(Gauge_Conf *GC, Geometry const *const geo,
                     GParam const *const param) {
  FILE *fp;
  int i, dimension, tmp_i;
  int err;
  ...
  fp = fopen(param->d_higgs_conf_file,
             "rb"); // open the configuration file in binary
  if (fp == NULL) {
    fprintf(stderr, "Error in opening the file %s (%s, %d)\n",
            param->d_conf_file, __FILE__, __LINE__);
    exit(EXIT_FAILURE);
    ...
    if (strncmp(md5sum_old, md5sum_new, 2 * MD5_DIGEST_LENGTH + 1) != 0) {
      fprintf(stderr,
              "The computed md5sum %s does not match the stored %s for the "
              "file %s (%s, %d)\n",
              md5sum_new, md5sum_old, param->d_higgs_conf_file, __FILE__,
              __LINE__);
      exit(EXIT_FAILURE);
    }
  }
...
void write_higgs_on_file(Gauge_Conf const *const GC, Geometry const *const geo,
                         GParam const *const param) {
...
// compute the md5sum of the higgs field and save it in res, that is a
// char[2*MD5_DIGEST_LENGTH]
void compute_md5sum_higgs(char *res, Gauge_Conf const *const GC,
                          Geometry const *const geo) {
  MD5_CTX mdContext;
  unsigned char c[MD5_DIGEST_LENGTH];
  long si, lex;
...
void alloc_diag_proj_stuff(Gauge_Conf *GC, Geometry const *const geo) {
  int err, dir;
  long r;

  // allocate diag_proj
  err = posix_memalign((void **)&(GC->diag_proj), (size_t)DOUBLE_ALIGN,
                       (size_t)geo->d_volume * sizeof(double));
  if (err != 0) {
  ...
  for (r = 0; r < geo->d_volume; r++) {
    err = posix_memalign((void **)&(GC->diag_proj[r]), (size_t)DOUBLE_ALIGN,
                         (size_t)STDIM * sizeof(double));
    if (err != 0) {
      fprintf(stderr, "Problems in allocating diag_proj (%s, %d)\n", __FILE__,
              __LINE__);
      exit(EXIT_FAILURE);
    ...
    for (dir = 0; dir < STDIM; dir++) {
      err =
          posix_memalign((void **)&(GC->diag_proj[r][dir]),
                         (size_t)DOUBLE_ALIGN, (size_t)NCOLOR * sizeof(double));
      if (err != 0) {
        fprintf(stderr, "Problems in allocating diag_proj (%s, %d)\n", __FILE__,
                __LINE__);
        exit(EXIT_FAILURE);
  ...
  // alloc u1_subg
  err = posix_memalign((void **)&(GC->u1_subg), (size_t)DOUBLE_ALIGN,
                       (size_t)geo->d_volume * sizeof(double));
  if (err != 0) {
  ...
  for (r = 0; r < geo->d_volume; r++) {
    err = posix_memalign((void **)&(GC->u1_subg[r]), (size_t)DOUBLE_ALIGN,
                         (size_t)STDIM * sizeof(double));
    if (err != 0) {
      fprintf(stderr, "Problems in allocating u1_subg (%s, %d)\n", __FILE__,
              __LINE__);
  ...
  // alloc uflag
  err = posix_memalign((void **)&(GC->uflag), (size_t)DOUBLE_ALIGN,
                       (size_t)geo->d_volume * sizeof(double));
  if (err != 0) {
  ...
  for (r = 0; r < geo->d_volume; r++) {
    err = posix_memalign((void **)&(GC->uflag[r]), (size_t)DOUBLE_ALIGN,
                         (size_t)STDIM * sizeof(double));
    if (err != 0) {
      fprintf(stderr, "Problems in allocating uflag (%s, %d)\n", __FILE__,
              __LINE__);
...
void free_diag_proj_stuff(Gauge_Conf *GC, Geometry const *const geo) {
  int dir;
  long r;
