...
// perform a complete update for correlated sampling
void update_corrsampling(Gauge_Conf *GC, Geometry const *const geo,
                         GParam const *const param, double *acc) {

#ifdef THETA_MODE
  fprintf(stderr,
          "THETA_MODE not yet implemented in the correlated sampling case, "
          "check everything "
          "(%s, %d)\n",
          __FILE__, __LINE__);
  exit(EXIT_FAILURE);
...
#pragma omp parallel for num_threads(NTHREADS) private(r)
    ...
    for (r = 0; r < (geo->d_volume) / 2; r++) {
      // heatbath(GC, geo, param, r, dir);
      a[r] += metropolis(GC, geo, param, r, dir, maxhits);
    }
