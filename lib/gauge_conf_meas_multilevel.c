...
void optimize_multihit_polycorr(Gauge_Conf *GC, Geometry const *const geo,
  ...
  err = posix_memalign((void **)&poly_array, (size_t)DOUBLE_ALIGN,
  ...
  for (mh = 1; mh < max_hit; mh++) {
    ...
    fprintf(datafilep, "%d  %.12g  %.12g (time:%g)\n", mh,
            poly_corr_abs * sqrt(mh), poly_corr_fluct * sqrt(mh), diff_sec);
...
void optimize_multilevel_polycorr(Gauge_Conf *GC, Geometry const *const geo,
  ...
  multilevel_polycorr(GC, geo, param, geo->d_size[0]);
...
#pragma omp parallel for num_threads(NTHREADS) private(r)
#endif
    for (r = 0; r < geo->d_space_vol; r++) {
      times_equal_TensProd(&(GC->ml_polycorr[0][0][r]),
                           &(GC->ml_polycorr[0][i][r]));
    }
  }
  ...
  poly_corr_abs = 0.0;
  for (r = 0; r < geo->d_space_vol; r++) {
    poly_array[r] = retr_TensProd(&(GC->ml_polycorr[0][0][r])) +
                    I * imtr_TensProd(&(GC->ml_polycorr[0][0][r]));
...
void optimize_multilevel_polycorr_with_higgs(Gauge_Conf *GC,
  ...
  multilevel_polycorr_with_higgs(GC, geo, param, geo->d_size[0]);
...
// perform the computation of the polyakov loop correlator with the multilevel
// algorithm
...
// perform the computation of the polyakov loop correlator with the multilevel
// algorithm for the theory with higgs fields
...
// print the value of the polyakov loop correlator that has been computed by
// multilevel
...
void perform_measures_tube_disc(Gauge_Conf *GC, Geometry const *const geo,
  ...
  multilevel_tube_disc(GC, geo, param, geo->d_size[0]);
...
#pragma omp parallel for num_threads(NTHREADS) private(r)
    ...
    for (r = 0; r < geo->d_space_vol; r++) {
      times_equal_TensProd(&(GC->ml_polycorr[0][0][r]),
                           &(GC->ml_polycorr[0][i][r]));
      times_equal_TensProd(&(GC->ml_polyplaq[0][r]),
                           &(GC->ml_polycorr[0][i][r]));
    }
  }
...
void perform_measures_tube_conn(Gauge_Conf *GC, Geometry const *const geo,
  ...
  multilevel_tube_conn(GC, geo, param, geo->d_size[0]);
...
#pragma omp parallel for num_threads(NTHREADS) private(r)
    ...
    for (r = 0; r < geo->d_space_vol; r++) {
      ...
      times_equal_TensProd(&(GC->ml_polyplaq[0][r]),
                           &(GC->ml_polycorr[0][i][r]));
      times_equal_TensProd(&(GC->ml_polyplaqconn[0][r]),
                           &(GC->ml_polycorr[0][i][r]));
    }
  }
