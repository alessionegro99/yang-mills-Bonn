...
void comp_MAG_gauge_transformation_U1(U1 X_links[2 * STDIM],
                                      ...
                                      double OverRelaxParam, U1 *G_mag) {
  (void)G_mag;
  (void)OverRelaxParam;
  (void)lambda;
  (void)X_links;
...
void comp_functional_fmag_U1(U1 X_links[2 * STDIM], double const lambda[1],
                             double *fmag) {
  (void)fmag;
  (void)X_links;
  (void)lambda;
...
void diag_projection_single_site_U1(Gauge_Conf *GC, U1 *link, long r, int dir) {
  double phi;
