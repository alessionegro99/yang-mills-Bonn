...
// A=b*B+c*C
void lin_comb_U1(U1 *A, double b, U1 const *const B, double c,
                 U1 const *const C);

// A=b*B^{dag}+c*C
void lin_comb_dag1_U1(U1 *A, double b, U1 const *const B, double c,
                      U1 const *const C);

// A=b*B+c*C^{dag}
void lin_comb_dag2_U1(U1 *A, double b, U1 const *const B, double c,
                      U1 const *const C);

// A=b*B^{dag}+c*C^{dag}
void lin_comb_dag12_U1(U1 *A, double b, U1 const *const B, double c,
                       U1 const *const C);
...
void diag_matrix_times_U1(U1 *restrict A, double const lambda[1],
                          U1 const *const restrict B);

// A=lambda*B^{dag} with lambda diagonal matrix
void diag_matrix_times_dag_U1(U1 *restrict A, double const lambda[1],
                              U1 const *const restrict B);
...
int print_on_binary_file_noswap_U1(FILE *fp, U1 const *const A) {
  ...
  if (err != 1) {
    fprintf(stderr,
            "Problem in binary writing on a file a U1 matrix (%s, %d)\n",
            __FILE__, __LINE__);
    return 1;
  }
...
void times_equal_complex_single_U1Vecs(U1Vecs *restrict A, double complex r,
                                       int j);
...
int print_on_binary_file_noswap_U1Vecs(FILE *fp, U1Vecs const *const A) {
  ...
  for (i = 0; i < NHIGGS; i++) {
    ...
    if (err != 1) {
      fprintf(stderr,
              "Problem in binary writing on a file a U1 vector (%s, %d)\n",
              __FILE__, __LINE__);
      return 1;
    }
    err = fwrite(&im, sizeof(double), 1, fp);
