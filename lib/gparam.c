...
void readinput(char *in_file, GParam *param) {
  ...
  if (input == NULL) {
    ...
    if (param->d_ml_step[0] != 0) {
      if (param->d_sizeg[0] % param->d_ml_step[0] ||
          param->d_sizeg[0] < param->d_ml_step[0]) {
        fprintf(stderr,
                "Error: size[0] has to be divisible by ml_step[0] and satisfy "
                "ml_step[0]<=size[0] (%s, %d)\n",
                __FILE__, __LINE__);
        exit(EXIT_FAILURE);
      }
      for (i = 1; i < NLEVELS; i++) {
        if (param->d_ml_step[i - 1] % param->d_ml_step[i] ||
            param->d_ml_step[i - 1] <= param->d_ml_step[i]) {
          fprintf(stderr,
                  "Error: ml_step[%d] has to be divisible by ml_step[%d] and "
                  "larger than it (%s, %d)\n",
    ...
    for (i = 0; i < STDIM; i++) {
      ...
      if (temp_i != 0) {
        ...
        fprintf(stderr,
                "When using OpenMP all the sides of the lattice have to be "
                "even! (%s, %d)\n",
                __FILE__, __LINE__);
        exit(EXIT_FAILURE);
      }
    ...
    if (err == 1) {
      fprintf(stderr,
              "Error: all sizes has to be larger than 1: the totally reduced "
              "case is not implemented! (%s, %d)\n",
              __FILE__, __LINE__);
    }
  }
}
...
// print parameters gaugebreaking
void print_parameters_gaugebreaking(GParam const *const param,
  ...
  fp = fopen(param->d_log_file, "w");
  fprintf(fp, "+-------------------------------------------------+\n");
  fprintf(fp, "| Simulation details for yang_mills_gaugebreaking |\n");
  fprintf(fp, "+-------------------------------------------------+\n\n");
...
void print_parameters_polycorr_higgs_long(GParam *param, time_t time_start,
  ...
  fprintf(fp, "epsilon_metro: %.10lf\n", param->d_epsilon_metro);
  fprintf(fp,
          "metropolis acceptance (different from zero only when the whole "
          "lattice is updated!): %.10lf\n",
          acc);
  fprintf(fp, "\n");
...
void print_parameters_higgs(GParam const *const param, time_t time_start,
  ...
  fp = fopen(param->d_log_file, "w");
  fprintf(fp, "+------------------------------------------------------+\n");
  fprintf(fp, "| Simulation details for yang_mills_correlated_sampling|\n");
  fprintf(fp, "+------------------------------------------------------+\n\n");
