#ifndef YM_LOCAL_OBC_C
#define YM_LOCAL_OBC_C

#include"../include/macro.h"

#include<stdio.h>
#include<stdlib.h>
#include<string.h>
#include<time.h>

#ifdef OPENMP_MODE
  #include<omp.h>
#endif

#include"../include/function_pointers.h"
#include"../include/gauge_conf.h"
#include"../include/geometry.h"
#include"../include/gparam.h"
#include"../include/random.h"

static int file_will_append(char const *filename, GParam const *param)
  {
  FILE *fp;

  if(param->d_start != 2)
    {
    return 0;
    }

  fp=fopen(filename, "r");
  if(fp!=NULL)
    {
    fclose(fp);
    return 1;
    }

  return 0;
  }


void real_main(char *in_file)
    {
    Gauge_Conf GC;
    Geometry geo;
    GParam param;

    char name[STD_STRING_LENGTH], aux[STD_STRING_LENGTH];
    int count, appendW, appendsW;
    FILE *datafilep, *datafileW, *datafilesW;
    time_t time1, time2;

    // to disable nested parallelism
    #ifdef OPENMP_MODE
      omp_set_nested(0);
    #endif

    // read input file
    readinput(in_file, &param);

    // initialize random generator
    initrand(param.d_randseed);

    appendW=file_will_append(param.d_Wloop_file, &param);
    appendsW=file_will_append(param.d_sWloop_file, &param);

    // initialize geometry
    init_geometry(&geo, param.d_sizeg);

    // open data files
    init_data_file(&datafilep, &param);
    init_Wloop_file(&datafileW, &param);
    init_sWloop_file(&datafilesW, &param);
    write_obc_measurement_headers(datafileW, datafilesW, &geo, &param,
                                  !appendW, !appendsW);

    // initialize gauge configuration
    init_gauge_conf(&GC, &geo, &param);

    // apply open boundary conditions in the spatial directions
    apply_obc_spatial(&GC, &geo);

    // montecarlo
    time(&time1);

    // measure the starting configuration, following the U1-OBC workflow
    perform_measures_localobs_obc(&GC, &geo, &param, datafilep, datafileW,
                                  datafilesW);

    // count starts from 1 to avoid problems using %
    for(count=1; count < param.d_sample + 1; count++)
       {
       update_obc(&GC, &geo, &param);

       if(count % param.d_measevery ==0 && count >= param.d_thermal)
         {
         perform_measures_localobs_obc(&GC, &geo, &param, datafilep,
                                       datafileW, datafilesW);
         }

       // save configuration for backup
       if(param.d_saveconf_back_every!=0)
         {
         if(count % param.d_saveconf_back_every == 0 )
           {
           // simple
           write_conf_on_file(&GC, &geo, &param);

           // backup copy
           write_conf_on_file_back(&GC, &geo, &param);
           }
         }

       // save configuration for offline analysis
       if(param.d_saveconf_analysis_every!=0)
         {
         if(count % param.d_saveconf_analysis_every == 0 )
           {
           strcpy(name, param.d_conf_file);
           sprintf(aux, "%ld", GC.update_index);
           strcat(name, aux);
           write_conf_on_file_with_name(&GC, &geo, name);
           }
         }
       }
    time(&time2);
    // montecarlo end

    // close data files
    fclose(datafilep);
    fclose(datafileW);
    fclose(datafilesW);

    // save configuration
    if(param.d_saveconf_back_every!=0)
      {
      write_conf_on_file(&GC, &geo, &param);
      }

    // print simulation details
    print_parameters_local_obc(&param, time1, time2);

    // free gauge configuration
    free_gauge_conf(&GC, &geo);

    // free geometry
    free_geometry(&geo);
    }


void print_template_input(void)
  {
  FILE *fp;
  int i;

  fp=fopen("template_input.in", "w");

  if(fp==NULL)
    {
    fprintf(stderr, "Error in opening the file template_input.in (%s, %d)\n", __FILE__, __LINE__);
    exit(EXIT_FAILURE);
    }
  else
    {
    fprintf(fp, "size");
    for(i=0; i<STDIM; i++)
      {
      fprintf(fp, " 4");
      }
    fprintf(fp, "\n");
    fprintf(fp,"\n");
    fprintf(fp, "beta 1.4\n");
    fprintf(fp, "theta 0\n");
    fprintf(fp,"\n");
    fprintf(fp, "sample    10\n");
    fprintf(fp, "thermal   0\n");
    fprintf(fp, "overrelax 4\n");
    fprintf(fp, "measevery 1\n");
    fprintf(fp, "monomeas  0  # monopole measurements are not used by this executable\n");
    fprintf(fp, "r0");
    for(i=1; i<STDIM; i++)
      {
      fprintf(fp, " 2");
      }
    fprintf(fp, "  # omit this line to use the spatial center\n");
    fprintf(fp,"\n");
    fprintf(fp, "start                   0  # 0=ordered  1=random  2=from saved configuration\n");
    fprintf(fp, "saveconf_back_every     0  # if 0 does not save, else save backup configurations every ... updates\n");
    fprintf(fp, "saveconf_analysis_every 0  # if 0 does not save, else save configurations for analysis every ... updates\n");
    fprintf(fp, "\n");
    fprintf(fp, "coolsteps  0     # number of cooling steps to be used\n");
    fprintf(fp, "coolrepeat 0     # number of times 'coolsteps' are repeated\n");
    fprintf(fp,"\n");
    fprintf(fp, "#output files\n");
    fprintf(fp, "conf_file   conf.dat\n");
    fprintf(fp, "data_file   dati.dat\n");
    fprintf(fp, "Wloop_file  Wloop.dat\n");
    fprintf(fp, "sWloop_file sWloop.dat\n");
    fprintf(fp, "mon_file    mon.dat\n");
    fprintf(fp, "log_file    log.dat\n");
    fprintf(fp, "\n");
    fprintf(fp, "randseed 0    #(0=time)\n");
    fclose(fp);
    }
  }


int main (int argc, char **argv)
    {
    char in_file[50];

    if(argc != 2)
      {
      printf("\nPackage %s version %s\n", PACKAGE_NAME, PACKAGE_VERSION);
      printf("Claudio Bonati %s\n", PACKAGE_BUGREPORT);
      printf("Usage: %s input_file\n\n", argv[0]);

      printf("Compilation details:\n");
      printf("\tN_c (number of colors): %d\n", NCOLOR);
      printf("\tST_dim (space-time dimensionality): %d\n", STDIM);
      printf("\tNum_levels (number of levels): %d\n", NLEVELS);
      printf("\n");
      printf("\tINT_ALIGN: %s\n", QUOTEME(INT_ALIGN));
      printf("\tDOUBLE_ALIGN: %s\n", QUOTEME(DOUBLE_ALIGN));

      #ifdef DEBUG
        printf("\n\tDEBUG mode\n");
      #endif

      #ifdef OPENMP_MODE
        printf("\n\tusing OpenMP with %d threads\n", NTHREADS);
      #endif

      #ifdef THETA_MODE
        printf("\n\tusing imaginary theta\n");
      #endif

      printf("\n");

      #ifdef __INTEL_COMPILER
        printf("\tcompiled with icc\n");
      #elif defined(__clang__)
        printf("\tcompiled with clang\n");
      #elif defined( __GNUC__ )
        printf("\tcompiled with gcc version: %d.%d.%d\n",
                __GNUC__, __GNUC_MINOR__, __GNUC_PATCHLEVEL__);
      #endif

      print_template_input();

      return EXIT_SUCCESS;
      }
    else
      {
      if(strlen(argv[1]) >= STD_STRING_LENGTH)
        {
        fprintf(stderr, "File name too long. Increse STD_STRING_LENGTH in include/macro.h\n");
        }
      else
        {
        strcpy(in_file, argv[1]);
        }
      }

    real_main(in_file);

    return EXIT_SUCCESS;
    }

#endif
