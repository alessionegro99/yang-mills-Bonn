...
// to disable nested parallelism
  ...
  while (count < max_count) {
    ...
    fprintf(datafilep, "# %.13lf  %.13lf  %.13lf  %.13lf\n", gftime,
            energy_clover, energy_clover * gftime * gftime, tch);
    if (energy_clover * gftime * gftime > 0.3) {
      ris = gftime - param.d_gfstep +
            (0.3 - energy_clover_old * gftime * gftime) * param.d_gfstep /
                (energy_clover * gftime * gftime -
                 energy_clover_old * gftime * gftime);
