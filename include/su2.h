...
//
// a complex 2-vector \Phi = (\phi_1+i\phi_2, \phi_3+i\phi_4) can be
// conveniently rewritten as a 2x2 matrix \varphi by using the auxilliary vector
// \tilde{\Phi}=i\sigma_2\Phi* and (see Montvay Muenster eq. 6.1)
// \varphi=( \tilde{\Phi} | \Phi ) = \phi_3 Id + i\sigma_1\phi_2
// +i\sigma_2\phi_1-i\sigma_3\phi_4 It is not difficult to show that
...
// A-=(r*B)
inline void minus_equal_times_real_Su2(Su2 *restrict A,
                                       Su2 const *const restrict B, double r) {
#ifdef DEBUG
  if (A == B) {
    fprintf(stderr, "The same pointer is used twice in (%s, %d)\n", __FILE__,
            __LINE__);
...
// A=b*B+c*C
inline void lin_comb_Su2(Su2 *restrict A, double b, Su2 const *const restrict B,
...
// A=b*B^{dag}+c*C
inline void lin_comb_dag1_Su2(Su2 *restrict A, double b,
                              Su2 const *const restrict B, double c,
...
// A=b*B+c*C^{dag}
inline void lin_comb_dag2_Su2(Su2 *restrict A, double b,
                              Su2 const *const restrict B, double c,
                              Su2 const *const restrict C) {
#ifdef DEBUG
  if (A == B || A == C || B == C) {
...
// A=b*B^{dag}+c*C^{dag}
inline void lin_comb_dag12_Su2(Su2 *restrict A, double b,
                               Su2 const *const restrict B, double c,
                               Su2 const *const restrict C) {
#ifdef DEBUG
  if (A == B || A == C || B == C) {
...
inline void times_equal_complex_Su2(Su2 *restrict A, double complex r) {
  ...
  if (fabs(cimag(r)) > MIN_VALUE) {
    fprintf(stderr,
            "Trying to multiply SU(2) matrix by a non-real number (%s, %d)\n",
            __FILE__, __LINE__);
    exit(EXIT_FAILURE);
  }
#endif
...
inline void times_equal_Su2(Su2 *restrict A, Su2 const *const restrict B) {
  ...
  A->comp[0] =
      a0 * B->comp[0] - a1 * B->comp[1] - a2 * B->comp[2] - a3 * B->comp[3];
  A->comp[1] =
      a0 * B->comp[1] + a1 * B->comp[0] - a2 * B->comp[3] + a3 * B->comp[2];
  A->comp[2] =
      a0 * B->comp[2] + a2 * B->comp[0] + a1 * B->comp[3] - a3 * B->comp[1];
  A->comp[3] =
      a0 * B->comp[3] + a3 * B->comp[0] - a1 * B->comp[2] + a2 * B->comp[1];
}
...
// A=B*C
inline void times_Su2(Su2 *restrict A, Su2 const *const restrict B,
                      Su2 const *const restrict C) {
#ifdef DEBUG
  if (A == B || A == C || B == C) {
    fprintf(stderr, "The same pointer is used twice in (%s, %d)\n", __FILE__,
  ...
  A->comp[0] = B->comp[0] * C->comp[0] - B->comp[1] * C->comp[1] -
               B->comp[2] * C->comp[2] - B->comp[3] * C->comp[3];
  A->comp[1] = B->comp[0] * C->comp[1] + B->comp[1] * C->comp[0] -
               B->comp[2] * C->comp[3] + B->comp[3] * C->comp[2];
  A->comp[2] = B->comp[0] * C->comp[2] + B->comp[2] * C->comp[0] +
               B->comp[1] * C->comp[3] - B->comp[3] * C->comp[1];
  A->comp[3] = B->comp[0] * C->comp[3] + B->comp[3] * C->comp[0] -
               B->comp[1] * C->comp[2] + B->comp[2] * C->comp[1];
}

// A=lambda*B with lambda diagonal matrix
inline void diag_matrix_times_Su2(Su2 *restrict A, double const lambda[2],
                                  Su2 const *const restrict B) {
  fprintf(stderr, "The function diag_matrix_times_Su2 cannot be defined using "
                  "the Pauli representation of SU(2) ");
  fprintf(stderr, "(%s, %d)\n", __FILE__, __LINE__);
  exit(EXIT_FAILURE);
...
inline void diag_matrix_times_dag_Su2(Su2 *restrict A, double const lambda[2],
                                      Su2 const *const restrict B) {
  fprintf(stderr, "The function diag_matrix_times_dag_Su2 cannot be defined "
                  "using the Pauli representation of SU(2) ");
  fprintf(stderr, "(%s, %d)\n", __FILE__, __LINE__);
  exit(EXIT_FAILURE);
...
// A=B^{dag}*C
inline void times_dag1_Su2(Su2 *restrict A, Su2 const *const restrict B,
                           Su2 const *const restrict C) {
#ifdef DEBUG
  if (A == B || A == C || B == C) {
    fprintf(stderr, "The same pointer is used twice in (%s, %d)\n", __FILE__,
...
// A=B*C^{dag}
inline void times_dag2_Su2(Su2 *restrict A, Su2 const *const restrict B,
                           Su2 const *const restrict C) {
#ifdef DEBUG
  if (A == B || A == C || B == C) {
    fprintf(stderr, "The same pointer is used twice in (%s, %d)\n", __FILE__,
...
// A=B^{dag}*C^{dag}
inline void times_dag12_Su2(Su2 *restrict A, Su2 const *const restrict B,
                            Su2 const *const restrict C) {
#ifdef DEBUG
  if (A == B || A == C || B == C) {
    fprintf(stderr, "The same pointer is used twice in (%s, %d)\n", __FILE__,
...
inline void unitarize_Su2(Su2 *restrict A) {
  ...
  p = A->comp[0] * A->comp[0] + A->comp[1] * A->comp[1] +
      A->comp[2] * A->comp[2] + A->comp[3] * A->comp[3];
  p = 1.0 / sqrt(p);
...
inline void TensProd_init_Su2(TensProd *restrict TP,
                              Su2 const *const restrict A1,
                              Su2 const *const restrict A2) {
#ifdef DEBUG
  if (A1 == A2) {
    fprintf(stderr, "The same pointer is used twice in (%s, %d)\n", __FILE__,
            __LINE__);
...
// A=B
inline void equal_Su2Vecs(Su2Vecs *restrict A,
                          Su2Vecs const *const restrict B) {
#ifdef __INTEL_COMPILER
  __assume_aligned(&(A->comp), DOUBLE_ALIGN);
  __assume_aligned(&(B->comp), DOUBLE_ALIGN);
#endif
...
// A-=B
inline void minus_equal_Su2Vecs(Su2Vecs *restrict A,
                                Su2Vecs const *const restrict B) {
#ifdef __INTEL_COMPILER
  __assume_aligned(&(A->comp), DOUBLE_ALIGN);
  __assume_aligned(&(B->comp), DOUBLE_ALIGN);
...
// A+=B
inline void plus_equal_Su2Vecs(Su2Vecs *restrict A,
                               Su2Vecs const *const restrict B) {
#ifdef __INTEL_COMPILER
  __assume_aligned(&(A->comp), DOUBLE_ALIGN);
  __assume_aligned(&(B->comp), DOUBLE_ALIGN);
...
inline void times_equal_real_single_Su2Vecs(Su2Vecs *restrict A, double r,
                                            int j) {
#ifdef __INTEL_COMPILER
  __assume_aligned(&(A->comp), DOUBLE_ALIGN);
#endif
...
inline void times_equal_complex_single_Su2Vecs(Su2Vecs *restrict A,
...
inline double re_scal_prod_Su2Vecs(Su2Vecs const *const restrict v1,
  ...
  for (i = 0; i < NHIGGS; i++) {
    ...
    ris += retr_Su2(
        &aux2); // the 0.5 that is due to the different normalization of
                // matrices and vectors (see text above the definition of
                // Su2Vecs) is not needed since retr = trace/2
  }

  return ris;
}

// real part of the scalar product re(v_1[a]^{\dag}v_2[b]) with a, b flavour
// indices
inline double re_scal_prod_single_Su2Vecs(Su2Vecs const *const restrict v1,
                                          Su2Vecs const *const restrict v2,
                                          int a, int b) {
#ifdef __INTEL_COMPILER
  __assume_aligned(&(v1->comp), DOUBLE_ALIGN);
  ...
  times_equal_dag_Su2(&aux2, &aux1);
  ris =
      retr_Su2(&aux2); // the 0.5 that is due to the different normalization of
                       // matrices and vectors (see text above the definition of
                       // Su2Vecs) is not needed since retr = trace/2
...
// v1=matrix*v2
inline void matrix_times_vector_single_Su2Vecs(Su2Vecs *restrict v1,
                                               Su2 const *const restrict matrix,
                                               Su2Vecs const *const restrict v2,
                                               int i) {
#ifdef __INTEL_COMPILER
  __assume_aligned(&(v1->comp), DOUBLE_ALIGN);
...
// v1=matrix*v2
inline void matrix_times_vector_all_Su2Vecs(Su2Vecs *restrict v1,
                                            Su2 const *const restrict matrix,
                                            Su2Vecs const *const restrict v2) {
#ifdef __INTEL_COMPILER
  __assume_aligned(&(v1->comp), DOUBLE_ALIGN);
...
inline void rotate_two_components_Su2Vecs(Su2Vecs *restrict v1,
  ...
  for (k = 0; k < 4; k++) {
    v1->comp[4 * i + k] =
        cos(angle) * v2->comp[4 * i + k] + sin(angle) * v2->comp[4 * j + k];
    v1->comp[4 * j + k] =
        -sin(angle) * v2->comp[4 * i + k] + cos(angle) * v2->comp[4 * j + k];
  }
}

// tensor product of two vectors
// Re(v1^{\dag} * aux * v2) = ReTr(aux * matrix)
inline void vector_tensor_vector_Su2Vecs(Su2 *restrict matrix,
                                         Su2Vecs const *const restrict v1,
                                         Su2Vecs const *const restrict v2) {
#ifdef __INTEL_COMPILER
  __assume_aligned(&(matrix->comp), DOUBLE_ALIGN);
  __assume_aligned(&(v1->comp), DOUBLE_ALIGN);
  ...
  times_equal_real_Su2(
      matrix, 0.5); // 0.5 is due to the different normalization of matrices and
                    // vectors see text above the definition of Su2Vecs
}

// initialize the flavour matrix with a vector
// FM[mf(i,j)]=\sum_{on_gauge}conj(v1[i])v1[j] - delta^{ij}/N
// i, j are the flavour indices
inline void init_FMatrix_Su2Vecs(FMatrix *restrict fmatrix,
                                 Su2Vecs const *const restrict v1) {
#ifdef __INTEL_COMPILER
  ...
  for (i = 0; i < NHIGGS; i++) {
    for (j = 0; j < NHIGGS; j++) {
      ...
      fmatrix->comp[mf(i, j)] +=
          (v1->comp[4 * i + 2]) * (v1->comp[4 * j + 1]) * I;
      fmatrix->comp[mf(i, j)] -=
          (v1->comp[4 * i + 1]) * (v1->comp[4 * j + 2]) * I;

      fmatrix->comp[mf(i, j)] +=
          (v1->comp[4 * i + 3]) * (v1->comp[4 * j + 0]) * I;
      fmatrix->comp[mf(i, j)] -=
          (v1->comp[4 * i + 0]) * (v1->comp[4 * j + 3]) * I;
    }
  }
...
// print on binary file without changing endiannes
int print_on_binary_file_noswap_Su2Vecs(FILE *fp,
                                        Su2Vecs const *const restrict A);

// print on binary file changing endiannes
int print_on_binary_file_swap_Su2Vecs(FILE *fp,
                                      Su2Vecs const *const restrict A);

// print on binary file in big endian format
int print_on_binary_file_bigen_Su2Vecs(FILE *fp,
                                       Su2Vecs const *const restrict A);
