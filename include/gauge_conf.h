...
typedef struct Gauge_Conf {
  ...
  GAUGE_GROUP **loc_poly;  // [d_size[0]/d_ml_step[NLEVELS-1]] [space_vol]
  ...
  double **uflag;      // [volume] [STDIM] this is used to check if the link has
                       // already been considered in the searching of the wraps
...
void read_tube_disc_stuff_from_file(Gauge_Conf const *const GC,
                                    Geometry const *const geo,
                                    GParam const *const param, int *iteration);
void compute_md5sum_tube_disc_stuff(
    char *res, // the lenght is 2*MD5_DIGEST_LENGTH
    Gauge_Conf const *const GC, Geometry const *const geo,
    GParam const *const param);
...
void read_tube_conn_stuff_from_file(Gauge_Conf const *const GC,
                                    Geometry const *const geo,
                                    GParam const *const param, int *iteration);
void compute_md5sum_tube_conn_stuff(
    char *res, // the lenght is 2*MD5_DIGEST_LENGTH
    Gauge_Conf const *const GC, Geometry const *const geo,
    GParam const *const param);
...
int DeGrand_current(Gauge_Conf *GC, Geometry const *const geo, long r, int dir);
...
void optimize_multihit_polycorr(Gauge_Conf *GC, Geometry const *const geo,
                                GParam const *const param, FILE *datafilep);
void optimize_multilevel_polycorr(Gauge_Conf *GC, Geometry const *const geo,
                                  GParam const *const param, FILE *datafilep);
void optimize_multilevel_polycorr_with_higgs(Gauge_Conf *GC,
                                             Geometry const *const geo,
                                             GParam const *const param,
                                             FILE *datafilep);
void perform_measures_polycorr(Gauge_Conf *GC, Geometry const *const geo,
                               GParam const *const param, FILE *datafilep);
...
void optimize_multilevel_polycorr_long(Gauge_Conf *GC,
                                       ...
                                       GParam const *const param,
                                       FILE *datafilep);
void perform_measures_polycorr_long(Gauge_Conf *GC, Geometry const *const geo,
                                    GParam const *const param, FILE *datafilep);

void perform_measures_tube_disc(Gauge_Conf *GC, Geometry const *const geo,
                                GParam const *const param, FILE *datafilep);
void perform_measures_tube_disc_long(Gauge_Conf *GC, Geometry const *const geo,
                                     GParam const *const param,
                                     FILE *datafilep);

void perform_measures_tube_conn(Gauge_Conf *GC, Geometry const *const geo,
                                GParam const *const param, FILE *datafilep);
void perform_measures_tube_conn_long(Gauge_Conf *GC, Geometry const *const geo,
                                     GParam const *const param,
                                     FILE *datafilep);

// in gauge_conf_multilevel.c
void multihit(Gauge_Conf const *const GC, Geometry const *const geo,
              GParam const *const param, long r, int dir, int num_hit,
              GAUGE_GROUP *G);

void update_for_multilevel(Gauge_Conf *GC, Geometry const *const geo,
                           GParam const *const param, int level);
void update_for_multilevel_with_higgs(Gauge_Conf *GC, Geometry const *const geo,
                                      GParam const *const param, int level);

void compute_local_poly(Gauge_Conf *GC, Geometry const *const geo,
                        GParam const *const param);
void multilevel_polycorr(Gauge_Conf *GC, Geometry const *const geo,
                         GParam const *const param, int dt);
void multilevel_polycorr_with_higgs(Gauge_Conf *GC, Geometry const *const geo,
                                    GParam const *const param, int dt);
void multilevel_polycorr_long(Gauge_Conf *GC, Geometry const *const geo,
                              GParam const *const param, int dt, int iteration);
void multilevel_polycorr_long_with_higgs(Gauge_Conf *GC,
                                         Geometry const *const geo,
                                         GParam const *const param, int dt,
                                         int iteration);

void compute_local_poly_and_plaq(Gauge_Conf *GC, Geometry const *const geo,
                                 GParam const *const param);
void multilevel_tube_disc(Gauge_Conf *GC, Geometry const *const geo,
                          GParam const *const param, int dt);
void multilevel_tube_disc_long(Gauge_Conf *GC, Geometry const *const geo,
                               GParam const *const param, int dt,
                               int iteration);
...
void compute_local_poly_plaq_and_plaqconn(Gauge_Conf *GC,
                                          Geometry const *const geo,
                                          GParam const *const param);
void multilevel_tube_conn(Gauge_Conf *GC, Geometry const *const geo,
                          GParam const *const param, int dt);
void multilevel_tube_conn_long(Gauge_Conf *GC, Geometry const *const geo,
                               GParam const *const param, int dt,
                               int iteration);

// in gauge_conf_upd.c
void calcstaples_wilson(Gauge_Conf const *const GC, Geometry const *const geo,
                        long r, int i, GAUGE_GROUP *M);
void calcstaples_wilson_nosum(Gauge_Conf const *const GC,
                              Geometry const *const geo, long r, int i,
                              GAUGE_GROUP *M);
void calcstaples_tracedef(Gauge_Conf const *const GC, Geometry const *const geo,
                          long r, int i, GAUGE_GROUP *M);
void compute_clovers(Gauge_Conf const *const GC, Geometry const *const geo,
                     int direction);
...
void calcstaples_with_topo_nosum(Gauge_Conf const *const GC,
                                 Geometry const *const geo,
                                 GParam const *const param, long r, int i,
                                 GAUGE_GROUP *M);

void heatbath(Gauge_Conf *GC, Geometry const *const geo,
              GParam const *const param, long r, int i);
void overrelaxation(Gauge_Conf *GC, Geometry const *const geo,
                    GParam const *const param, long r, int i);
int metropolis(Gauge_Conf *GC, Geometry const *const geo,
               GParam const *const param, long r, int i, int numhits);
int metropolis_with_tracedef(Gauge_Conf *GC, Geometry const *const geo,
                             GParam const *const param, long r, int i,
                             int numhits);
...
void update_obc(Gauge_Conf *GC, Geometry const *const geo,
                GParam const *const param);
void update_obc_alt(Gauge_Conf *GC, Geometry const *const geo,
                    GParam const *const param);
...
void gradflow_RKstep(Gauge_Conf *GC, Gauge_Conf *helper1, Gauge_Conf *helper2,
                     Geometry const *const geo, double dt);
void ape_smearing(Gauge_Conf *GC, Geometry const *const geo, double alpha,
                  int n);

void heatbath_with_higgs(Gauge_Conf *GC, Geometry const *const geo,
                         GParam const *const param, long r, int i);
void overrelaxation_with_higgs(Gauge_Conf *GC, Geometry const *const geo,
                               GParam const *const param, long r, int i);
void calcstaples_for_higgs(Gauge_Conf *GC, Geometry const *const geo, long r,
                           GAUGE_VECS *staple);
void overrelaxation_for_higgs(Gauge_Conf *GC, Geometry const *const geo,
                              long r);
int metropolis_for_higgs(Gauge_Conf *GC, Geometry const *const geo,
                         GParam const *const param, long r);
void update_with_higgs(Gauge_Conf *GC, Geometry const *const geo,
                       GParam const *const param, double *acc);
